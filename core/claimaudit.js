...
function buildClaimAudit(osDir, taskId) {
  ...
  L.push('');
  // この仕事は申告と台帳の突き合わせ＝分類であり、routing表の T1.use_for がまさにそれ。
  // 実測（T020の同一briefing）: T1の判定は8件すべてT2と一致し、消費は101k→65k（-36%）。
  // 判定（llm_judge）を同じ理屈でT1に落としてはいけない — あちらは成果物と実装を
  // 自分で読み直す仕事で、削ると検出力が落ちる。
  L.push('**この監査は T1（cheap）のモデルで実行してよい**（台帳との突き合わせ＝分類のため。');
  L.push('実測: 同一briefingでT2と判定が全件一致し、消費は36%少なかった）。');
  L.push('');
