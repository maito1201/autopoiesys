...
## 0-a. この装置は何を埋めるのか（設計判断の基準）

autopoiesysが埋めるのは**汎用モデル単体の機能欠損**である。
仮説はこうである — 今のモデルは、それ単体では「特定ドメインのタスクを日々行い、
記憶と経験を再利用して人間を超える知性的行動に至る」という目的行動において、
次の機能を構造的に欠いている。

**評価の単位はモデル単体ではなく〈モデル＋この装置〉である。**
目指すのは「装置と一緒なら正しく振る舞う」ことであって、「装置が無くても自力で判断できる」
ことではない。この線引きが設計判断を決める:

- 主戦場は**判断の瞬間に押し出す機構**である。「必要なら引きに来い」に任せた層は運用で死ぬ
  （実測: 決定層は3文脈・15タスクのあいだ一度も閉じなかった）。装置の価値は、
  必要な問いを必要な時点で供給することにあり、モデルに覚えさせることではない
- 「重みは変わらないのだから意味がない」という評価はこの基準では誤りである。
  複合体の振る舞いが変われば、それが知性の向上そのものである
- 逆に、**装置を迂回した状態でモデルを測るのは無効な測定である。** Skillを呼ばずに
  行われた仕事の失敗は、モデルの知性の不足ではなく複合体の運用の失敗として扱う。
  ただしその瞬間を担当する器官がそもそも無いなら、それは装置の欠損であり、
  この区別を曖昧にすると「使い方が悪い」は責任転嫁の口実になる

| 欠損 | 何が起きるか | 供給する部品 |
|---|---|---|
| セッションをまたぐ持続がない | 昨日の結論が今日には無い | World Model（追記専用のevents.jsonl） |
| 日々の再来が見えない | 同種の仕事を毎回「初めて」としてやる | タスク類型（class / class_fp） |
| 想起が保証されない | 自分が何を思い出せていないかを知らないまま判断する | `task new`/`task brief` の自動想起 / `decision recall` / 方針層の発火 / Reasoning Context |
| 経験が蒸留されない | 生ログが腐り、教訓にならない | type: lesson と `task consolidate`（開示の強制） |
| 結果が信念に書き戻らない | 自分の選択がうまくいったかを知る経路が無く、同じ判断を毎回ゼロからやる | outcome記録 → 方針のコンパイルと自動撤回 / 教訓への helped・misled 書き戻し |
| 完了の自己申告が当てにならない | 「できました」で終わる | 独立評価とNext Action Engine（完了を認定するのはOS） |
| 反証が構造化されていない | 外れた前提が黙って生き残る | Failure状態機械 / 方針の自動撤回 / 反証された教訓の想起除外 / 検出力テスト付きGolden Task |
| 成長が測れない | 「良くなっている」が主張のまま | growth（類型ごとの試行系列） |
| 指示が無いと止まる | 未解決の仕事が転がったまま | agenda（OSの状態から次の仕事を決定的に導出） |
| 文脈が有限で欠落する | 長い仕事ほど落ちる | 最小Subgraph化とcontext_logの実測 |
| 作った層が使われないまま残る | テストは全通過・実運用の記録は0件のまま誰も気づかない | agendaの `dead_organ`（記録が0件の器官を名指し。使うか捨てるかは指示しない） |

**新しい部品を作るときは、まずこの表のどの行を埋めるのかを名指しする。**
名指しできない部品は作る理由が無い。

この表を1本につなぐと、装置の中心にあるループはこうなる。

```
タスクをやる（class付き）
  → 経験が1行の教訓に蒸留される（task consolidate が開示を強制）
  → 次に同種のタスクをやるとき、教訓が黙っていても届く（task new が想起を押し付ける）
  → 使った教訓が効いたか外れたかが記録される（helped/misled → evidenceの極性リンク）
  → 外れの記録が上回った教訓は想起から外れる（反証の自動化）
  → 類型ごとの試行系列で、回数を重ねて良くなっているかを見る（growth）
```

人間の知性が成し得ていること — 経験を抽象化して再利用可能な思考様式にし、
判断のたびに一から考え直さない — の機械版がこのループである。
成立しているかは growth の系列と教訓の helped/misled 集計だけで判定する。
**教訓の件数・想起の回数は成功指標にしない**（たくさん思い出したことは、
役に立ったことを意味しない）。

そして**トークン節約それ自体を設計目標にしない。** トークン経済が効くのは単価ではなく
**文脈容量**という制約の側であり、安さは正しい設計の帰結として現れる。
...
（goal / domain / objectives / success_criteria / constraints / optimization / sources / notes）。**
autonomy.escalate_on は廃止した（F006: どのコードも読まない死んだ宣言だった。
昇格の実体は config.routing.escalation・goal監査・Failure台帳にある）。
...
`situation` / `options` / `chosen` / `criteria` / `expected_outcome` を載せて追記し、
結果は元をsupersedeせず `type: outcome` を追記して `derived_from` で張る
（決定の記録そのものは書き換えない）。`result: unmet` はFailureループへの起票を促す。
「保存して終わる」なら、それは知識ではなくログである（§26④）。

**支援対象はAI自身であって人間ではない。** 第1版はレビュー期限という日付を持ち、
期限切れを人間に催促する帳簿だった。カレンダー上の日付はAIの判断の契機にならない。
AIにとっての契機は**再来**であり、`decision new` は書き込む前に同じ判断の場の過去を
必ず突き返す。読むかどうかを実行者の判断に委ねない。判断の場は `situation` の文字列だけで
同定する（選択肢は同定に使わない）。完全一致しない場合でも、語が重なる過去の決定は
`near` として併せて返す — 場の切り方が細かすぎて再来しないことは、経験が無いことを意味しない。

### 方針層（直感） — モデル単体に無い「想起と書き戻し」を供給する

**この装置が埋めているのはモデルの機能欠損であって、トークン単価ではない。**
トークンを使わないこと自体を目的に置くと、行き着く先は小さなAIの再実装になる。
autopoiesysが供給するのは、汎用モデル単体では構造的に持てない次の2つである。

- **判断の場に立ったときに、過去の選択と結果が必ず届くこと。**
  モデルは自分が何を想起できていないかを知らない。想起を実行者の判断に委ねると、
  一番必要なときに一番落ちる
- **結果が信念に書き戻ること。** モデルは自分の選択がうまくいったかを知る経路を持たない。
  外から書き戻さない限り、同じ判断は毎回ゼロからやり直される

人間が貧弱な脳でAIより良い判断をするのは、過去の知識と経験を抽象化して再利用可能な
思考様式にし、判断のたびに一から考え直さないからである。機械側でこれに対応するのが
**反復して結果が伴った決定を決定的な方針へ畳み込み、以後は想起を強制すること**である
（`core/policy.js` → `.os/rules/policy-*.yaml`）。計算はハッシュ・索引・照合だけで、
推論を経ないため非決定性が無い。安いのはその帰結であって、設計理由ではない
（トークン経済が効くのは単価ではなく**文脈容量**という制約の側である）。

3層で構成する。(1)再来検出＝決定を書こうとした瞬間に過去を突き返す
(2)方針＝条件を満たしたら畳み込み、以後は選択を返す
(3)反証＝`unmet` が出たら**自動で**撤回する。3層目が無いと方針は反証不能な信念になるので、
撤回は裁量にしない。方針に反する選択が `met` になった場合も撤回し、
そのときは判断の場そのものを凍結する（場を分ける条件が situation に書かれていない証拠）。

この設計自体が仮説であり、棄却条件を先に置く: 方針が一度も発火しない（再来が起きない）、
または方針の下で下した判断の `unmet` 率が熟慮した判断より高い（直感が熟慮より悪い）なら、
この層は不要か有害である。`metrics.policy.outcomes` の2列がその判定に使う唯一の材料で、
**発火数と節約トークン量は成功指標にしない**。

**この棄却条件は実際に発火した（F011）。** 3文脈・15タスクを経た時点で、記録された決定6件に対し
結果の記録は0件、方針は0件、発火は0回だった。ただし調査の結果、原因は「再来が起きない領域だった」
ことではなく、**再来を検出する装置が壊れていた**ことだった。判断の場の同定に選択肢の文字列を
含めていたため、同一の場でも選択肢の書き方が違えば別の場になっていた。したがって場の同定は
situation だけで行い（選択肢は審議の中身であって場の同一性ではない）、記録済みの決定は
読み出し時に situation から引き直して統合する（台帳は追記専用のまま書き換えない）。

**押しつける層は動き、引きに来させる層は死ぬ。** 同じ期間に、教訓の層は task 登録時の想起で
黙って押しつける経路を持ち、配信56件の機械記録を作っていた。決定の層は `decision recall` を
実行者が自発的に呼ぶことだけを頼りにしており、記録は0だった。この対比を受けて、過去の決定は
想起に混ぜて配り（配信IDは `context_log.jsonl` に残る）、結果が未記録のまま完了したタスクの決定は
運用ヒントと `agenda` が出し続ける。**棄却条件そのものは撤回していない** — 場の同定を直した後も
方針が発火しないなら、そのときは層の側が誤りである。
...
さらに記録から escalation シグナル（同一evaluatorのUNCERTAIN2回連続 / 同じ状態への判定の食い違い /
未消化の未知fingerprint）を検出し、昇格先のtierを config.yaml の routing 表から引く。
食い違いだけは全PASSのDONEも上書きする（最新のPASSを採ると、覆った理由を調べずに完了になる）。
**食い違いは「いまの状態を見た判定どうし」で判定する** — 最後の成果物登録以降の
llm/human判定に限る。判定の並びだけを見ると、手順6が指示する FAIL → 修正 → PASS が
矛盾と読まれ、正直に是正したタスクほど完了に到達できなくなる（F012）。
